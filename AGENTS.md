...
- **JavaScript only** — no TypeScript, no CSS files, no CSS modules
- **Inline styles** via JS objects — no external stylesheets
...
- Environment variables for client config:
  - `PUBLIC_APPWRITE_ENDPOINT`, `PUBLIC_APPWRITE_PROJECT_ID`, `PUBLIC_APPWRITE_DATABASE_ID`, `PUBLIC_APPWRITE_TRIPS_COLLECTION_ID`, `PUBLIC_APPWRITE_PARTICIPANTS_COLLECTION_ID`, `PUBLIC_APPWRITE_PROPOSALS_COLLECTION_ID`
...
- Database helpers are named: `listTrips`, `getTrip`, `createTrip`, `updateTrip`, `deleteTrip`, `joinTrip`, `leaveTrip`, `createProposal`, etc.
